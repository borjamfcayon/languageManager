...
export const ClassUsers = ({ page, userInfo }: { page: number; userInfo?: IUser }) => {
    const { data: classes, isLoading: loadingClasses } = useGetAllClassByIds(userInfo?.class ?? []);
    ...
    return (
        <div>
            {classes[page] && <p className="text-lg font-bold">{classes[page].language} </p>}
